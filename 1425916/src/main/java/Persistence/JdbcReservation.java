...
public class JdbcReservation implements DAOReservation {
    ...
    public long getDayDiff( int orderId){
        int dayDiff=0;
        conn = ConnectionFactory.getConnection();
        try {
           preparedStatement=conn.prepareStatement("SELECT DATEDIFF(day, Select RESERVATIONFROM from RESERVATION where orderId= ?, Select RESERVATIONTO from RESERVATION where orderId=?)");
           preparedStatement.setInt(1, orderId);
           preparedStatement.setInt(2, orderId);
           resultSet=preparedStatement.executeQuery();
            while(resultSet.next()){
               dayDiff=resultSet.getInt(1);
            }
            preparedStatement.close();
        }catch (SQLException e){
            e.printStackTrace();
    ...
    public ArrayList<Reservation> findAllIntersectReservations(String dateFrom, String dateTo) {
        ...
        conn = ConnectionFactory.getConnection();
        try {
            preparedStatement=conn.prepareStatement("Select * from RESERVATION where ReservationFrom >= ? and ReservationTo <= ?");
